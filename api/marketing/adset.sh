...
resp=$( . get.sh "${path}" "name=${name}&status=${status}&optimization_goal=${optimization_goal}&billing_event=${billing_event}&promoted_object=${promoted_object}&bid_amount=${bid_amount}&daily_budget=${daily_budget}&campaign_id=${campaign_id}&targeting={'geo_locations':{'countries':['US','GB']}, 'page_types':['mobilefeed'], 'user_os':['ios']}&access_token=${access_token}" )
